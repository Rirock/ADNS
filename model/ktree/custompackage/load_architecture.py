...
class ktree_gen(nn.Module):
    ...
    def __init__(self, ds='mnist', Activation="relu", Sparse=True,
                 Input_order=None, Repeats=1, Padded=False, Input_size=10, Output_size=1, device="cpu"):
        ...
        # If using 16x16 datasets...
            ...
            # Use input sized tree
            if Input_size > 10:
                self.k = [Input_size, 8, 4, 2, 1]
            elif Input_size > 5:
                self.k = [Input_size, 4, 2, 1]
            else:
                self.k = [Input_size, 2, 1]
        else:
        ...
        for j in range(self.Repeats):
            ...
            for i in range(len(self.k)-1):
                ...
                self._modules[name].bias = nn.Parameter(torch.zeros_like(self._modules[name].bias))#.to(device)
                # Use custom method to re-initialize the layer weights and create freeze mask for that layer
                self._modules[name].weight.data, freeze_mask = self.initialize(self._modules[name], device)
        ...
        self.root = nn.Linear(Repeats, Output_size)
    ...
    def forward(self, x):
        ...
        # output = self.sigmoid(y_out[-1])
        return(output)
    
    def initialize(self, layer, device):
        ...
        if self.Sparse:
            ...
            weights = torch.nn.init.normal_(weights, mean=0.0, std=math.sqrt(2/(weights.shape[1]*density))).to(device)
            ...
            weights = torch.nn.init.normal_(weights, mean=0.0, std=math.sqrt(2/(weights.shape[1]))).to(device)
        
        # Generate freeze mask for use in training to keep weights initialized to zero at zero
        mask_gen = torch.zeros_like(weights).to(device)
    ...
    def reset_parameters(self):
        self.__init__(ds=self.ds, Activation=self.Activation, Sparse=False, Input_order=None, Repeats=1, Padded=False, Input_size=self.Input_size, Output_size=self.Output_size, device=self.device)


class ktree_gen2(nn.Module):
    '''
    k-Tree neural network
    '''
    
    def __init__(self, ds='mnist', Activation="relu", Sparse=True,
                 Input_order=None, Repeats=1, Padded=False, Input_size=10, device="cpu"):
        super(ktree_gen2, self).__init__()
        self.input_size = Input_size
        self.linear1 = nn.Linear(Input_size, 4)
        self.linear2 = nn.Linear(Input_size, 2)
